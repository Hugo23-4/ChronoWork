...
export default function AdminRequestsManager() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      let query = supabase.from('solicitudes').select('*', { count: 'exact' }).order('created_at', { ascending: false });
      ...
      if (queryError) { setError(`Error BD: ${queryError.message}`); console.error('Error fetching solicitudes:', queryError); }
        ...
        if (empIds.length > 0) {
          const { data: emps } = await supabase.from('empleados_info').select('id, nombre_completo, dni').in('id', empIds);
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (!data || data.length === 0) setError(`No hay solicitudes${filter !== 'todos' ? ` en estado "${filter}"` : ''}.`);
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
      ...
      <div className="p-4 border-b border-gray-100">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-3">
          <div>
            <h5 className="font-bold text-navy mb-1">Solicitudes del Personal</h5>
            <small className="text-slate-400">
              {loading ? 'Cargando...' : error ? `⚠️ ${error}` : `✅ ${totalCount} solicitud${totalCount === 1 ? '' : 'es'} encontrada${totalCount === 1 ? '' : 's'}`}
            </small>
          </div>
            ...
            {[
              ...
              <button key={f.value} onClick={() => setFilter(f.value)}
                className={cn('px-3 py-1.5 rounded-full text-xs font-bold border-[1.5px] cursor-pointer transition-all',
                  filter === f.value ? 'bg-navy text-white border-navy' : 'bg-transparent text-slate-500 border-gray-200 hover:border-gray-300')}>
                {f.label}
              </button>
            ))}
          </div>
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50/80">
            <tr>
              <th className="pl-5 py-3 text-slate-400 text-xs uppercase font-bold text-left">Empleado</th>
              <th className="py-3 text-slate-400 text-xs uppercase font-bold text-left">Tipo</th>
              <th className="py-3 text-slate-400 text-xs uppercase font-bold text-left">Fechas</th>
              <th className="py-3 text-slate-400 text-xs uppercase font-bold text-left">Archivo</th>
              <th className="py-3 text-slate-400 text-xs uppercase font-bold text-left">Estado</th>
              <th className="pr-5 py-3 text-slate-400 text-xs uppercase font-bold text-right">Acciones</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr><td colSpan={6} className="text-center py-10">
                ...
                <p className="text-slate-400 mb-1">{error || 'No hay solicitudes'}</p>
                {!error && <small className="text-slate-400">Las solicitudes aparecerán aquí cuando los empleados las creen</small>}
              </td></tr>
            ) : (
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              requests.map((req: any) => (
                <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="pl-5 py-3">
                    <div className="font-bold text-navy text-sm">{req.empleados_info?.nombre_completo || 'Desconocido'}</div>
                    <small className="text-slate-400 text-xs">{req.empleados_info?.dni}</small>
                  </td>
                  <td className="py-3">{typeBadge(req)}</td>
                  <td className="py-3">
                    <small className="text-navy text-sm">
      ...
      <div className="md:hidden p-3">
        {loading ? (
          ...
          <div className="grid gap-3 pb-6">
            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
            {requests.map((req: any) => (
              <div key={req.id} className="bg-white rounded-2xl p-3.5 shadow-sm border border-gray-100">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h6 className="font-bold text-navy text-sm mb-0">{req.empleados_info?.nombre_completo || 'Desconocido'}</h6>
                    <small className="text-slate-400 text-xs">{req.empleados_info?.dni}</small>
                  </div>
