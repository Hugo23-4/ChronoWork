  ...
  return (
    <div className="animate-fade-up pb-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="font-bold mb-0 text-navy font-[family-name:var(--font-jakarta)]">Generación de Informes</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-5">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 h-full">
            <h5 className="font-bold mb-4 text-navy">1. Configurar Parámetros</h5>

            <div className="mb-4">
              <label className="block text-[0.65rem] font-bold text-slate-400 mb-2 uppercase tracking-widest">PERIODO A AUDITAR</label>
              <div className="relative">
                <input type="text" className="w-full px-4 py-3 border border-gray-200 rounded-lg bg-gray-50 text-sm font-bold outline-none" value="01 Oct 2026 - 31 Oct 2026" readOnly />
                <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
              </div>
            </div>

            <div className="mb-4">
              <label className="block text-[0.65rem] font-bold text-slate-400 mb-2 uppercase tracking-widest">TIPO DE REPORTE</label>
              {[
                ...
                <div key={opt.value} onClick={() => setFormat(opt.value)}
                  className={cn('mb-2 rounded-xl cursor-pointer transition-all border-2 p-3 flex items-center gap-3',
                    format === opt.value ? 'border-amber-400 bg-amber-500/10' : 'border-gray-100 bg-gray-50 hover:border-gray-200')}>
                  <opt.Icon className={cn('w-5 h-5 shrink-0', opt.iconColor)} />
                  <div>
                    <h6 className="font-bold mb-0 text-navy text-sm">{opt.label}</h6>
                    <small className="text-slate-400">{opt.desc}</small>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-7 hidden lg:block">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 h-full">
            <h5 className="font-bold mb-4 text-navy">2. Vista Previa del Documento</h5>
