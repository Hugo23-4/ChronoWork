...
const LocationPicker = dynamic(() => import('@/components/admin/LocationPicker'), {
  ssr: false,
  loading: () => (
    <div className="flex flex-col items-center justify-center h-full bg-gray-50 text-slate-400 dark:text-zinc-500 rounded-2xl">
...
export default function CentrosTrabajoPage() {
  ...
  useEffect(() => { fetchSedes(); }, [profile?.empresa_id]);

  const fetchSedes = async () => {
    if (!profile?.empresa_id) return;
    const { data } = await supabase.from('sedes').select('*').eq('empresa_id', profile.empresa_id).eq('activo', true).order('created_at', { ascending: false });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const payload = { nombre: formData.nombre, direccion: formData.direccion, latitud: formData.lat, longitud: formData.lng, radio_metros: formData.radio, empresa_id: profile?.empresa_id };
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    if (error) {
      alert('No se pudo eliminar el centro: ' + (error.message.includes('violates foreign key') ? 'Hay empleados asignados a esta sede. Reasígnales primero.' : error.message));
      return;
    }
    fetchSedes();
  ...
  return (
    <div className="animate-fade-up -mx-4 -mt-4 min-h-[calc(100vh-100px)]">
      {/* Header */}
      <div className="bg-white dark:bg-zinc-900 px-4 md:px-5 py-4 border-b border-gray-100 dark:border-zinc-800 sticky top-0 z-40">
        <div className="flex justify-between items-center">
          <div>
            ...
            <h2 className="font-bold text-navy dark:text-zinc-100 text-xl md:text-2xl font-[family-name:var(--font-jakarta)]">
              {showMobileMap ? (formData.id ? 'Editar Ubicación' : 'Nueva Ubicación') : 'Centros de Trabajo'}
            </h2>
            {!showMobileMap && <p className="text-slate-400 dark:text-zinc-500 text-sm mt-1 hidden lg:block">Configura las zonas GPS permitidas para el fichaje.</p>}
          </div>
      ...
      <div className="grid lg:grid-cols-12 gap-0 lg:gap-5 p-4 md:p-5 min-h-[600px]">
        ...
        <div className={cn('lg:col-span-4', showMobileMap ? 'hidden lg:block' : 'block')}>
          <div className="flex justify-between items-center mb-3">
            <small className="text-slate-500 dark:text-zinc-400 font-bold bg-gray-100 px-2.5 py-1 rounded-full text-xs border border-gray-200 dark:border-zinc-700">{sedes.length} ACTIVOS</small>
          </div>
          <div className="grid gap-3 pb-6">
            {loading ? (
              ...
              <div className="text-center py-10">
                ...
                <h6 className="font-bold text-slate-500 dark:text-zinc-400 mb-1">No hay centros creados</h6>
                <p className="text-sm text-slate-400 dark:text-zinc-500">Crea el primero para empezar.</p>
              </div>
            ) : (
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        <h6 className="font-bold text-navy dark:text-zinc-100 text-sm truncate pr-2">{centro.nombre}</h6>
                      ...
                      <div className="flex items-center text-slate-400 dark:text-zinc-500 text-xs mt-0.5 mb-2">
                        <MapPin className="w-3 h-3 mr-1" />
                        <span className="truncate">{centro.direccion || 'Ubicación GPS'}</span>
                      </div>
                      <span className="bg-gray-100 text-slate-600 text-xs px-2.5 py-0.5 rounded-full font-medium border border-gray-200 dark:border-zinc-700">
        ...
        <div className={cn('lg:col-span-8', !showMobileMap ? 'hidden lg:block' : 'block')}>
          <div className="bg-white dark:bg-zinc-900 shadow-sm rounded-2xl overflow-hidden h-[600px] flex flex-col">
            ...
            <div className="p-5 bg-white flex-1">
              ...
              <div className="grid md:grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="text-[0.7rem] font-bold text-slate-400 dark:text-zinc-500 mb-1 block uppercase tracking-wider">Nombre del Centro</label>
                  <input type="text" className="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-800 border-none rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-chrono-blue/20"
                    placeholder="Ej: Oficina Central" value={formData.nombre} onChange={e => setFormData({ ...formData, nombre: e.target.value })} />
                </div>
                <div>
                  <label className="text-[0.7rem] font-bold text-slate-400 dark:text-zinc-500 mb-1 block uppercase tracking-wider">Dirección (Opcional)</label>
                  <input type="text" className="w-full px-3 py-2.5 bg-gray-50 dark:bg-zinc-800 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-chrono-blue/20"
                    placeholder="Calle, número..." value={formData.direccion} onChange={e => setFormData({ ...formData, direccion: e.target.value })} />
                </div>
              </div>
              <div className="mb-3">
                <div className="flex justify-between mb-1">
                  <label className="text-[0.7rem] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Radio de Fichaje</label>
